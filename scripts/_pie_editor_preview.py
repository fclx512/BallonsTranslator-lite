...
Outputs:
    scripts/_pie_editor_preview.png
"""
...
_TMP_CFG = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                        "_pie_editor_preview_config.json")
shared.CONFIG_PATH = _TMP_CFG
if os.path.exists(_TMP_CFG):
    os.remove(_TMP_CFG)
load_config(_TMP_CFG)

# A half-ring list menu to stress the config-page preview.
pcfg.pie_menus = [{
    "id": "edit",
    "name": "Edit Menu",
    "trigger": "Tab",
...
def main():
    ...
    pm = scroll.grab()
    pm.save(os.path.join("scripts", "_pie_editor_preview.png"))
    page.grab().save(os.path.join("scripts", "_pie_editor_preview_full.png"))
    print("saved scripts/_pie_editor_preview.png")
    print("saved scripts/_pie_editor_preview_full.png")
