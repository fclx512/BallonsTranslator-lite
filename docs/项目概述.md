...
BallonsTranslator-lite 通过 MCP（Model Context Protocol）将项目操作暴露给 AI 编程助手（如 Claude Code）。MCP 服务器是一个独立的子进程，通过 stdio 传输与客户端通信。

```
MCP 客户端（Claude Code 等）
    ↕ stdio JSON-RPC
mcp_server/main.py（FastMCP 服务器）
    ↕
mcp_server/tools.py（工具注册）
    ↕
utils/ai_tools.py（工具执行）
    ↕
utils/proj_compact.py（项目序列化）
    ↕
utils/proj_imgtrans.py（ProjImgTrans）
```
...
├── main.py           # 服务器初始化与启动
├── project_manager.py # 项目生命周期管理（打开/关闭/保存）
...
| 分类 | 工具 | 功能 |
|------|------|------|
| 项目管理 | `open_project` | 加载项目到内存 |
| | `save_project` | 将当前项目写回磁盘 |
| | `close_project` | 释放项目内存 |
| | `get_state` | 获取服务器/项目状态 |
| 读取 | `list_pages` | 获取所有页面的轻量索引 |
| | `read_pages` | 读取指定页的详细数据（含文本块） |
| | `search_blocks` | 在原文/译文中搜索文本块 |
| | `get_config` | 读取项目全局配置 |
| | `get_page_info` | 获取页面图片尺寸和元信息 |
| 写入 | `set_font` | 批量设置字体样式 |
| | `set_color` | 批量设置颜色属性 |
| | `set_layout` | 批量设置排版参数 |
...
`utils/proj_compact.py` 负责将 `ProjImgTrans` 项目序列化为 AI 可读的 JSON 格式：

- `build_index(proj)` — 返回项目索引（页名、尺寸、块数、字符统计）
- `build_detail(proj, page_indices)` — 返回指定页的详细数据（含原文/译文/样式）
- `build_paginated_detail(proj, page_indices)` — 分页版详情（适配大项目的 token 限制）
...
依赖安装在 `ballontrans_pylibs_win/` 中，启动前需确保 `pcfg` 已初始化（`load_config()`）。

---

> **历史**：此 MCP 架构取代了上游内置的 AI 聊天面板（`ai_chat_panel.py`、`ai_controller.py`、`ai_change_review.py` 等 7 个文件已被移除）。不再内置 AI 对话 UI，改为标准 MCP 接口供外部 AI 工具控制。
