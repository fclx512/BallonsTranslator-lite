...
> 本文档对应设置面板的所有功能项。面板导航分为两大区域：**DL Module**（深度学习模块设置）和 **General**（通用设置），左侧为树形导航（`ConfigTable`），右侧为详细设置区域。
...
| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| Model Path | 下拉框+文件选择 | `data/models/ysgyolo_yolo26_2.0.pt` | 模型文件路径，可手动输入或浏览选择 |
...
| Vertical Latin/Digits Length | 滑块(0-9) | 竖排文字中连续拉丁字母/数字显示为正排（縦中横）的长度阈值，0 表示禁用 |
| Exclude Fonts... | 按钮 | 打开字体排除对话框，可从列表中隐藏不需要的字体 |
| Max Font Size (px) | 数字选择器(10~1000) | 渲染时最大允许的字号（像素） |

---

### 3. 性能 (Performance)

> 2026-07-08 新增。管理渲染质量与界面动画。

| 显示名 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| Animation | 下拉框 | Auto (match display) | 控制 UI 过渡动画流畅度。Auto 匹配屏幕刷新率；60/30 FPS 限制帧率以降低 GPU 负载；Off 禁用所有动画 |
| Text Rendering | 下拉框 | Smooth (bitmap cache) | 控制画布文字渲染方式。Crisp — 始终使用矢量路径渲染，任意缩放均清晰，拖动大块文字可能卡顿；Smooth — 预渲染为位图缓存，拖动/滚动流畅，缩放后短暂模糊直至缓存重建 |
| Show decorations while resizing | 勾选框 | 关 | 勾选后拖拽/缩放文字块时保留描边和阴影。关闭可获最大重绘帧率 |
...
| 显示名 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| Window Fit | 勾选框 | 关 | 打开项目时自动缩放图片适配窗口 |
| Also fit when switching pages | 勾选框 | 关 | 切换页面时也自动适配窗口（仅在 Window Fit 开启时可用） |

#### 快捷键 (Keyboard Shortcuts)

> 每个操作可绑定多个快捷键。点击 "Edit Shortcuts…" 按钮打开编辑器，在卡片上点击 **+** 添加按键绑定，点击 **x** 移除，点击 **Reset** 恢复默认。
...
#### 预设值 (Combo Box Presets)

用于文字排版面板下拉栏的快捷数值列表（逗号分隔）：

| 预设 | 默认值 |
|------|--------|
| Font Size | 6, 8, 10, 12, 14, 18, 24, 30, 36, 48, 60, 72, 96, 120 |
| Line Spacing | 1.0, 1.1, 1.2, 1.5, 2.0 |
| Letter Spacing | 0.0, 0.5, 1.0, 1.5, 2.0 |
| Stroke Width | 0.1, 0.15, 0.2, 1.0 |
| Opacity | 1.0, 0.8, 0.6, 0.4, 0.2 |

#### 原图对比 (Original Compare)

| 显示名 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| Preset (%) | 数字选择器 | 20 | 使用原图对比快捷键时，译文背景的透明度预设值 |

#### 其他

| 显示名 | 类型 | 说明 |
|--------|------|------|
| Show sequence number on text blocks | 勾选框 | 在画布文字块左上角显示序号 |
