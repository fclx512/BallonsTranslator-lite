...
对 BallonsTranslator-lite 现有 UI 层进行全面审查后，发现若干 bug、交互缺失、以及主题系统/响应式设计方面的技术债。本计划按优先级分三轮实施。已完成项已移除（closeEvent 冻结、Reject All 按钮）。
...
### 1.2 修复 `on_merge_finished` 裸 except（Bug）
...
| 文件 | 涉及改动 |
|------|----------|
| `ui/mainwindow.py` | Bug 修复（1.1, 1.2）、面板宽度（3.1）、加载进度（3.2） |
| `ui/ai_chat_panel.py` | 进度显示（1.3）、重试按钮（1.4）、取消（1.5）、面板宽度（3.1） |
| `ui/ai_change_review.py` | 列宽自适应（3.5） |
| `utils/ai_controller.py` | 进度信号（1.3）、取消支持（1.5） |
