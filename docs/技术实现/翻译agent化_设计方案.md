...
- 主窗口左侧栏 LeftBar 的工作台图标(`ui/mainwindowbars.py::LeftBar` 的 workbenchChecker,面板为 `ui/mainwindow.py` 内嵌左栏 `GlossaryAgentPanel`——与全局搜索同槽位的加宽版,开合走宽度动画 `ui/mainwindow.py::on_set_workbench_widget`);
