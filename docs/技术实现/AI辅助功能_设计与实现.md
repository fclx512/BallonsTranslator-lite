...
| 9 | **工作台 UI 逐项优化**（用户 2026-09-18 启动，进行中） | **已做**：导航两级化（D42）、底部状态条（D43）、容器 `WA_StyledBackground`（底色 QSS 原先空转）、D44 浮层形态收口（自适应大小／点画布或 Esc 关／无关闭钮／标题报"哪一页哪个框"）、② 无选中行时「跳到画布」禁用、参数控件改 `ConfigCheckBox`（裸 `QCheckBox` 掉主题指示器）、数值框单位后缀随模式走且 px 不翻译、参数一变导航计数即时刷新（`ui/workbench_batch_view.py::BatchTaskView` 的 `plan_changed`）、术语／剧情页补一行说明、提示语全面收短（含设置页「工作台（临时）」两条备注）。**待改**（同一类"逻辑没说清"的问题，逐条来）：① 空态/首次进入时日志区只有「草稿已载入：术语 0 条」，与当前任务无关，宜按任务分栏或只在草稿任务显示；③ 执行钮文字带计数（「覆盖 0 页」）在无候选时不如直接禁用＋写明原因；④ 任务参数控件（扩张量等）与执行钮分两行、参数改完要等 180ms 防抖才重规划，观感上像"点了没反应"；⑤ 浮层是否记忆位置（现只记忆"用户调过尺寸"这一状态，位置每次锚左上角）。 |
