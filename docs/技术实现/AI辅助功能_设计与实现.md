...
容器＝`ui/glossary_agent_panel.py::GlossaryAgentPanel`，**三段式**：① 两级导航 `ui/glossary_agent_panel.py::WorkbenchTaskNav`（D42：**一级＝任务大类**，用管线阶段名「文字与 OCR／图像修复／翻译」；**二级＝该大类下的任务 chip**，切大类时落到上次在该大类里用过的任务）→ ② 候选列表（批量任务为列表；术语／剧情是各自的草稿页）→ ③ 执行行（无勾选即禁用，批量执行前一律走告知弹窗）。
「批处理与审批是同一任务的两端，不分家」——D20 定的就是这个。

**默认顺序＝用户工作流顺序**：① 误识别清理 → ② 合并相邻框 → ③ 框扩张 → ④ 简单背景修复 → ⑤ 术语提取 → ⑥ 剧情摘要。
大类归属：文字与 OCR＝误识别清理／合并相邻框／框扩张，图像修复＝背景修复，翻译＝术语表／剧情。前四项属「问题清理」任务族、计数「还有 N 个未处理」缀在二级 chip 上，同时按大类求和缀在一级页签上（切到别的大类时仍看得见还有活）；术语／剧情是探索性任务、无「未处理」概念，不缀数。**运行时不门禁**：跳步时弹窗提示「还有 N 个未处理的该步骤标记」（可勾选不再提示，写 `utils/config.py::ProgramConfig` 的 `workbench_warn_skip_order`）。
...
**四个批量任务**（D3／D5／D7／D8／D16／D30／D31／D32／D33／D34／D36／D41／D45）
...
| D42 | **导航两级化**：一级＝任务大类（用**管线阶段名**「文字与 OCR／图像修复／翻译」，不是任务名——按「误识别清理」找功能的用户找不到入口），二级＝该大类下的任务 chip；切大类落到该大类上次用过的任务；未处理计数两级都缀。大类标签的翻译上下文用 `WorkbenchTaskNav`（`GlossaryAgentPanel` 里 "Translation" 已被术语表列头占为「译文」）。 |
...
| 5 | 设置页「工作台」页两个数值项是否即时生效 | 只能实机验（该页已于 2026-09-21 去掉「临时」字样、作为正式页保留）。 |
| 6 | ~~设置页「工作台（临时）」页的排版归并~~ | **已结（2026-09-21）**：用户拍板**保留为正式页「工作台」**——不并入应用页（这两项是批量任务参数，与那页两条提示开关不同族），只去掉「临时」字样，并把两行的 `note=` 收成一句话（实测数字本来就在上面的「参数已实测定」表里）。 |
| 7 | §15 末尾的延后项（翻译「自动」档、自动排版） | 明确延后，涉及新增功能。 |
| 8 | 长尾未做项 | 多选批量打标与行级标签、交付门禁（导出前检查残留疑点标签）、指示标签被管线消费时的可视化、处理中锁定工具栏失焦隐藏、逐行 OCR 校正不做可疑行预选。 |
| 9 | **工作台 UI 逐项优化**（用户 2026-09-18 启动，进行中） | **已做**：导航两级化（D42）、底部状态条（D43）、容器 `WA_StyledBackground`（底色 QSS 原先空转）、D44 浮层形态收口（自适应大小／点画布或 Esc 关／无关闭钮／标题报"哪一页哪个框"）、② 无选中行时「跳到画布」禁用、参数控件改 `ConfigCheckBox`（裸 `QCheckBox` 掉主题指示器）、数值框单位后缀随模式走且 px 不翻译、参数一变导航计数即时刷新（`ui/workbench_batch_view.py::BatchTaskView` 的 `plan_changed`）、术语／剧情页补一行说明、提示语全面收短（含设置页工作台页两条备注，2026-09-21 再收成一句话）。**待改**（同一类"逻辑没说清"的问题，逐条来）：① 空态/首次进入时日志区只有「草稿已载入：术语 0 条」，与当前任务无关，宜按任务分栏或只在草稿任务显示；③ 执行钮文字带计数（「覆盖 0 页」）在无候选时不如直接禁用＋写明原因；④ 任务参数控件（扩张量等）与执行钮分两行、参数改完要等 180ms 防抖才重规划，观感上像"点了没反应"；⑤ 浮层是否记忆位置（现只记忆"用户调过尺寸"这一状态，位置每次锚左上角）。 |
