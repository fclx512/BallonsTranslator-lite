...
| `stroke_size` | `font_size × ff.stroke_width`（与 `effect_renderer.py:325` 一致；旧代码 `×0.5` 是错的） |
| `shadow_blur/distance/opacity/angle` | 沿用 `effect_renderer` 公式：blur/offset = 参数×font_size；strength→百分比；angle 为 PS 光源角（offset 反向 +180°） |
