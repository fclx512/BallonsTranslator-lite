        ...
        if hasattr(_torch, "cuda") and not _torch.cuda.is_available():
            try:
                ...
                if any(
                    ...
                    print("\n" + "=" * 60)
                    print(
                        f"WARNING: Detected Blackwell GPU ({_gpu_name}) but CUDA is not available!"
                    )
                    print(
                        "The installed PyTorch was compiled for an older CUDA version"
                    )
...
def prepare_environment():
    ...
    try:
        ...
        if any(
            name in _gpu_name
            for name in ["RTX 5090", "RTX 5080", "RTX 5070", "RTX 5060", "RTX 50"]
        ):
