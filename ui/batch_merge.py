"""批量合并相邻文本框（规划 §6.3、D7／D8／D30～D33／D40）。

用户场景（规划 §4「逐页人工审校 OCR」）：惯用的检测模型**一行一框**（竖排
...
**分组判据**（规划 §6.3 的实测口径：「同页 + 行高相近 + 重叠 > 0.6」）。本
...
**口径提醒（2026-09-16）**：规划 §6.3 的实测脚本随 ``tmp/`` 清理已不存在，
...
@dataclass
class MergeGroup:
    """一个候选组（不含块对象——跨页编排的回调寻址原则，见 AI辅助功能规划 §8.9）。"""
