...
class TextAdvancedFormatPanel(PanelArea):
    ...
    def __init__(self, panel_name: str, config_name: str, config_expand_name: str, on_format_changed: Callable):
        ...
        btn_style = ("QPushButton { border: 1px solid palette(mid); border-radius: 4px; "
        ...
        self.linespacing_type_combobox = SmallComboBox(
            parent=self,
            options=[self.tr("Proportional"), self.tr("Distance")]
        )
    ...
    def _update_effect_btns(self, font_format: FontFormat):
        ...
        if has_shadow:
            ...
            self.shadow_btn.setStyleSheet(
                base + f" QPushButton {{ background: rgba({r},{g},{b},40); border-color: rgb({r},{g},{b}); }}"
            )
        else:
        ...
        if has_grad:
            ...
            self.gradient_btn.setStyleSheet(
                base + f" QPushButton {{ background: qlineargradient(x1:0, y1:0, x2:1, y2:0, "
