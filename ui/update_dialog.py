"""Update-release dialog with Markdown release notes.

Ported from upstream ``dmMaze/BallonsTranslator`` (ui/update_dialog.py); the
sponsor-message section was dropped for this fork.
"""
...
# Section heading used as a language marker inside release notes Markdown
# (matching text, not UI copy — kept as escapes to avoid the hardcoded-Chinese scan).
ZH_NOTES_HEADING = "\u66f4\u65b0\u8bf4\u660e"
    ...
    if selected_heading is None:
        ...
        for heading_index, heading_level, heading_title in headings:
            is_localized_boundary = (
                ...
                or heading_title == ZH_NOTES_HEADING
            )
...
class UpdateReleaseDialog(QDialog):
    ...
    def __init__(
        ...
        self.release_notes.setHorizontalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
        self.release_notes.setVerticalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
