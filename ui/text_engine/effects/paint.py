...
Port of upstream ``ballontranslator/ui/text_engine/effects/paint.py``.
Fork deviation: the TexturePaint branch (``colorize_texture_paint_rgba`` and
its premultiply helpers) is not ported — image-texture effect cards are
deferred (计划第六节"后续待办"); re-add both together with the Image/
texture cards.
"""
