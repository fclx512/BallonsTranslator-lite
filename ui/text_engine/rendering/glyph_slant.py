...
Port of upstream v1.5.9 ``text_engine/rendering/glyph_slant.py`` (Stage 4).
Local adaptations:

- import prefixes use the local package layout;
- ``vertical_rotation_chars`` is read from the local
  ``VerticalTextDocumentLayout`` (added in Stage 4 node D);
- the two rotation decisions (``_vertical_line_placement`` and the
  ``angle == 0`` mask branch) carry the local tate-chu-yoko guard: a
  ``tatechuyoko`` character record suppresses vertical rotation, matching the
  local ``SceneTextLayout.draw`` semantics.
"""
...
class GlyphSlantLayoutRenderer:
    ...
    def _tatechuyoko_record(self, block_number: int, char_offset: int) -> dict:
        """Read the local per-character layout record for tate-chu-yoko."""
        records = getattr(self.layout, 'per_char_records', None)
        if records is None:
            return {}
        return records[block_number].get(char_offset, {})
    ...
    def _vertical_line_placement(
        ...
        if (
            char in self.layout.vertical_rotation_chars
            and not self._tatechuyoko_record(block_number, char_offset).get(
                "tatechuyoko"
            )
        ):
        ...
        while block.isValid():
            ...
            for line_number in range(layout.lineCount()):
                ...
                try:
                    if (
                        char in self.layout.vertical_rotation_chars
                        and not self._tatechuyoko_record(
                            block_number, char_offset
                        ).get("tatechuyoko")
                    ):
                        line_x, line_y = line.x(), line.y()
