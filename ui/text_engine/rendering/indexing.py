...
Port of upstream v1.5.9 ``text_engine/rendering/indexing.py`` (verbatim).
Stage 4 transformed-glyph rendering depends on these; the Stage 3
``TextEffectRenderer`` port only needs ``_utf16_length`` / ``_utf16_slice``
for its gradient FormatRange machinery.
"""
