"""Selection-scoped editing sessions for composable text transforms (Stage 5).

Local port of upstream v1.5.10 ``transforms/editor.py``
``TextTransformEditSession`` — the session owns transform targets, previews
and undo-command boundaries; the transform panel (Stage 5 node H) and the
scene controls (node I) talk only to the session, never to ``TextBlkItem``.

Local differences from upstream:

* ``controls`` is optional (``None`` when no panel is attached yet); every
  panel interaction is guarded, so the session state machine is testable
  without a real widget.
* Upstream's ``C.active_format`` global slot and ``host`` (text panel)
  dependency are dropped. State is read/written through
  ``item.blk.fontformat`` only; an optional ``global_format`` attribute
  keeps the no-items path available for the future panel without coupling.
* The scene-control dispatch keeps upstream's ``hasattr`` guard: the local
  canvas gains ``bind_text_grid_control`` / ``bind_text_projective_control``
...
class TextTransformEditSession:
    ...
    def _current_states(self):
        ...
        if self.global_format is not None:
            return [self._state_for_format(self.global_format)]
        return []
    ...
    def _sync_transform_controller(self) -> None:
        ...
        if selected is not None and len(self.items) == 1:
            ...
            if (
                ...
                and hasattr(canvas, 'bind_text_grid_control')
            ):
            ...
            if (
                ...
                and hasattr(canvas, 'bind_text_projective_control')
            ):
    ...
    def _commit_states(self, before, after) -> None:
        if not self.items:
            if (
                self.global_format is not None
                and before
                and before[0] != after[0]
            ):
                self.global_format.text_transform = after[0].stack
                self.global_format.glyph_slant_angle = after[0].glyph_slant_angle
    ...
    def refresh_controls(self, refresh_shape=True) -> None:
        ...
        elif self.global_format is not None:
            self.controls.set_transform(
                self._state_for_format(self.global_format)
            )
        if refresh_shape:
    ...
    def remove_transform(self, index: int) -> None:
        ...
        if (
            not before
            or not self._has_common_stack_shape(before)
            or index < 0
    ...
    def detach_scene_owner(self) -> None:
        """Drop target ownership before a page/scene switch."""
        self.cancel_control_previews()
        self.items = []
        self.selected_index = None
        self._sync_transform_controller()
