...
Stage 3 port of upstream v1.5.9 ``text_engine/effect_renderer.py``
(``TextEffectRenderer``).  Neutral-state behaviour (empty transform stack +
``glyph_slant_angle == 0``) must be pixel-identical to the pre-port local
``textitem`` implementation.  The transformed (non-neutral) branches keep
their upstream structure verbatim but are unreachable in Stage 3.

Localization notes (compared with upstream):
- ``paint_item`` neutral path only resets the option state and forwards to
...
class TextEffectRenderer:
    ...
    def paint_item(self, painter: QPainter, option, widget: QWidget, base_paint) -> None:
        ...
        if self._text_transform_is_neutral():
            # The host ``_paint_native`` owns the local composition order
            # (background before text via SourceOver, border/badge last) and
            # reads ``background_pixmap`` from this renderer.
        ...
        *,
        ...
        if paint_shadow:
            ...
            try:
                ...
                if paint_stroke and self.fontformat.shadow_include_stroke:
                    self.paint_stroke(
