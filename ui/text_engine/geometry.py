...
class TextItemGeometryController:
    ...
    def absolute_rect(self, max_h=None, max_w=None, qrect=False):
        ...
        Math mirrors the pre-port local ``absBoundingRect`` exactly
        (round/ceil integer conversion, max_h/max_w clamping), which many
        callers (OCR masks, project save, snapping) rely on.
        """
        rect = self.logical_rect()
    ...
    def _resize_standard(
        ...
        *,
        ...
        # 本地语义（零回归关键）：仅 vertical / Right(align_top_right) 做边缘
        # 保持补偿；Left / Center 均不补偿（对应本地 textitem.set_size 的
        # `if align_c or align_tl: pass` 分支）。不补偿意味着保中心的 pos_shift
        # 照常应用——Left 与 Center 行为一致，sceneBoundingRect 中心不变
        # （位置随之平移）。上游 `if not align_center` 会让 Left 也做补偿，
        # 得到"左上角位置不变、中心移动"的相反行为，移植时不可照搬。
        if item.fontformat.vertical:
            align_top_right = True
        else:
            align_top_right = item.fontformat.alignment == TextAlignment.Right
    ...
    def paint_item(self, painter, option, widget, base_paint) -> None:
        ...
        if mapper is None or renderer is None:
            # Neutral path: forward through the effect renderer stub (Stage 3
            # replaces it; the stub calls base_paint directly).
