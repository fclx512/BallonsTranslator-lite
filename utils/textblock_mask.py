...
def letter_calculator(img, mask, bground_rgb, show_process=False):
    ...
    # rgb to grey
    aver_bground_rgb = (
        0.299 * bground_rgb[0] + 0.587 * bground_rgb[1] + 0.114 * bground_rgb[2]
    )
    thresh_low = 127
...
def textrgb_calculator(img, text_mask, show_process=False):
    ...
    if show_process:
        colored_text_board = (
            np.zeros((img.shape[0], img.shape[1], 3), dtype=np.uint8) + 127
        )
...
def canny_flood(img, show_process=False, inpaint_sdthresh=10, **kwargs):
    ...
    if scaleR != 1:
        ...
        orimg = np.copy(img)
        img = cv2.resize(
            img, (int(w * scaleR), int(h * scaleR)), interpolation=cv2.INTER_AREA
        )
    h, w = img.shape[0], img.shape[1]
    img_area = h * w
    ...
    cons, hiers = cv2.findContours(
        detected_edges, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_NONE
    )
    ...
    outer_area, _, _, rect = cv2.floodFill(
        ballon_mask,
        mask=None,
        seedPoint=seedpnt,
        flags=4,
    ...
    for ii in range(2):
        ...
        bgarea1, _, _, rect = cv2.floodFill(
            mask,
            mask=None,
            seedPoint=(0, 0),
            flags=4,
        ...
        bgarea2, _, _, rect = cv2.floodFill(
            mask,
            mask=None,
            seedPoint=(detected_edges.shape[1] - 1, detected_edges.shape[0] - 1),
            flags=4,
            newVal=(127),
            loDiff=(difres, difres, difres),
            upDiff=(difres, difres, difres),
    ...
    if bground_aver[0] != -1:
        letter_aver, threshed = letter_calculator(
            img, mask, bground_aver, show_process=show_process
        )
        if letter_aver[0] != -1:
    ...
    if show_process:
        ...
        show_img_by_dict(
            {"outermask": ballon_mask, "detect": detected_edges, "mask": mask}
        )
...
def connected_canny_flood(
    img, show_process=False, inpaint_sdthresh=10, apply_strokewidth_check=0, **kwargs
):
    ...
    def find_outermask(img):
        ...
        ballon_mask = (
            255 - cv2.threshold(ballon_mask - 127, 1, 255, cv2.THRESH_BINARY)[1]
        )
    ...
    def ccctest(img, crop_r=0.1):
        ...
        if img.shape[0] > maxh:
            ...
            im = cv2.resize(
                img,
                (int(img.shape[1] * scaleR), int(img.shape[0] * scaleR)),
        ...
        num_labels, labels, stats, centroids, pseduo_outermask = find_outermask(
            cv2.threshold(
        ...
        for bgr_ind in range(4):
            ...
            ret, thresh = cv2.threshold(
                channel, 1, 255, cv2.THRESH_OTSU + cv2.THRESH_BINARY
            )
    ...
    if apply_strokewidth_check > 0:
        text_mask = strokewidth_check(
            text_mask, labels, num_labels, stats, debug_type=show_process - 1
        )

    text_color = textrgb_calculator(img, text_mask, show_process=show_process)
    inner_rect = cv2.boundingRect(
        cv2.findNonZero(cv2.dilate(text_mask, (3, 3), iterations=1))
    )
    inner_rect = [ii for ii in inner_rect]
    inner_rect.append(-1)
