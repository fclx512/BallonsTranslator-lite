...
def enlarge_window(rect, im_w, im_h, ratio=2.5, aspect_ratio=1.0) -> List:
    ...
    rect = np.array(
        [x1 - delta_w, y1 - delta, x2 + delta_w, y2 + delta], dtype=np.int64
    )
...
def color_difference(rgb1: List, rgb2: List) -> float:
    ...
    diff = cv2.cvtColor(color1, cv2.COLOR_RGB2LAB).astype(np.float64) - cv2.cvtColor(
...
def extract_ballon_region(
    ...
    x1, y1, x2, y2 = (
        ballon_rect[0],
        ballon_rect[1],
        ballon_rect[2] + ballon_rect[0],
        ballon_rect[3] + ballon_rect[1],
    )
    if enlarge_ratio > 1:
        x1, y1, x2, y2 = enlarge_window(
            [x1, y1, x2, y2],
            img.shape[1],
            img.shape[0],
            enlarge_ratio,
            aspect_ratio=ballon_rect[3] / ballon_rect[2],
        )
    ...
    ballon_area, _, _, rect = cv2.floodFill(
        ballon_mask,
        mask=None,
        seedPoint=seedpnt,
        flags=4,
