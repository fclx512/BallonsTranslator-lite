...
def split_step0(span, thresh, sumby_yaxis, thresh2=None) -> list[TextSpan]:
    candidate_pnts = (
        np.where(sumby_yaxis[span.top : span.bottom] > thresh)[0] + span.top
    ).tolist()
    span_list = []
    ...
    for pnt_ind in range(len(candidate_pnts) - 1):
        if candidate_pnts[pnt_ind + 1] - candidate_pnts[pnt_ind] > stride_tol:
            if not span0.set_bottom(candidate_pnts[pnt_ind]):
                continue
            span_list = split_step1(
                span0, span_list, thresh=thresh2, sumby_yaxis=sumby_yaxis
            )
...
def split_step1(span, span_list, thresh=None, sumby_yaxis=None):
    if thresh is None:
        ...
        if subspan_list is not None:
            ...
            if (
                maxspan.height / minspan.height > 2.5
                or sum_height / span.height < 0.3
                or len(subspan_list) == 1
            ):
                subspan_list = None
...
def shrink_span_list(src_img, span_list, shrink_vert_space=True, shrink_hor_space=True):
    ...
    if shrink_hor_space:
        ...
        left_var, middle_var = (
            np.var(np.array(left_pnts)),
            np.var(np.array(middle_pnts)),
        )
...
def find_span(span_list, max_or_min=max, key="height"):
    if key == "height":
        return max_or_min(
            enumerate(span_list), key=(lambda x: span_list[x[0]].height), default=-1
        )
    else:
        return max_or_min(
            enumerate(span_list), key=(lambda x: span_list[x[0]].width), default=-1
        )
...
def split_textblock(
    ...
    if crop_ratio > 0:
        img, height, width = crop_img(
            src_img, crop_ratio=crop_ratio, clip_width=clip_width, dilate=dilate
        )
    else:
