...
def sw_calculator(mask, canny_img, gradient_x, gradient_y, show_process=False):
    ...
    if show_process:
        drawborder = np.zeros(
            (canny_img.shape[0], canny_img.shape[1], 3), dtype=np.uint8
        )
    ...
    sample_pnt_num = 150
    sample_step = (
        total_pnt_num / sample_pnt_num if total_pnt_num > sample_pnt_num else 1
    )
    ...
    while cur_pnt_ind < total_pnt_num:
        ...
        valid, dx, dy = calculate_derivatives(
            gradient_x[start_y][start_x], gradient_y[start_y][start_x]
        )
        ...
        if valid:
            ...
            while True:
                ...
                if canny_img[tmp_cury][tmp_curx] == 0:
                    valid, dx_t, dy_t = calculate_derivatives(
                        gradient_x[tmp_cury][tmp_curx], gradient_y[tmp_cury][tmp_curx]
                    )
                    if not valid:
                        break
                    if np.arccos(-dx * dx_t + -dy * dy_t) < np.pi / 2.0:
                        ...
                        ray_arr[3] = tmp_cury
                        ray_arr[4] = np.sqrt(
                            (start_x - tmp_curx) ** 2 + (start_y - tmp_cury) ** 2
                        )
                    break
                cur_x += dx
                cur_y += dy
