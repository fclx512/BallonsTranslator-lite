...
def create_error_dialog(
    exception: Exception, error_msg: str = None, exception_type: str = None
):
    """
    ...
    show_exception = (
        exception_type_empty or exception_type not in shared.showed_exception
    )
    ...
    if show_exception:
        ...
        if not shared.HEADLESS:
            shared.create_errdialog_in_mainthread(
                error_msg, detail_traceback, exception_type
            )
...
def create_info_dialog(
    ...
    if not shared.HEADLESS:
        shared.create_infodialog_in_mainthread(
            {
                "info_msg": info_msg,
                "btn_type": btn_type,
                "modal": modal,
