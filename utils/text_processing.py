...
PKUSEG_PUNCSET = {' ', '.', '　'}
PKUSEGPATH = 'data/pkusegscores.json'
PKUSEGSCORES = None
CHSEG = None


def seg_to_chars(text: str) -> list:
    text = text.replace('\n', '')
    return [c for c in text]


def seg_eng(text: str) -> list:
...
def seg_ch_pkg(text: str) -> list:
    ...
    if CHSEG is None:
        try:
            ...
            # No segmenter available — degrade to per-character segments so
            # the caller keeps working (2026-08-21, engine 2a landing).
            CHSEG = False
...
def seg_text(text: str, lang: str):
    delimiter = ''
