"""
ContextBatchTranslator — lightweight batch translator using AI assistant config.

Created on-the-fly by the Run dialog when "Context Translation (beta)" is enabled.
Not registered as a translator module — reads api_config and translation_prompt
directly from the AI assistant (AiController), no separate profile management.
"""
...
class ContextBatchTranslator:
    ...
    set_project, finalize) while calling the LLM with the AI assistant's
    ...
    def _describe(self, pname, pidx):
        ...
        for bidx, blk in enumerate(blks):
            ...
            entries.append(e)
        return {"pidx": pidx, "name": pname, "translated": done, "blocks": entries}
