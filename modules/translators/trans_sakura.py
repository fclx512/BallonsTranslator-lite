...
class SakuraDict:
    ...
    def _load_galtransl_dic(self, dic_path: str) -> None:
        ...
        for line in dic_lines:
            ...
            if len(sp) < 2:
                continue
            src, dst, *info = sp
            gpt_dict.append({"src": src, "dst": dst, "info": info[0] if info else None})
        gpt_dict_text_list = [
            f"{gpt['src']}->{gpt['dst']}{' #' + gpt['info'] if gpt['info'] else ''}"
            for gpt in gpt_dict
        ]
    ...
    def _load_sakura_dict(self, dic_path: str) -> None:
        ...
        for line in dic_lines:
            ...
            gpt_dict_text_list.append(f"{src}->{dst}{' #' + info if info else ''}")
        self.dict_str = "\n".join(gpt_dict_text_list)
        self.logger.info(
    ...
    def get_dict_str_within_text(
        self, text: str, force_apply_dict: bool = False
    ) -> str:
        """
    ...
    def save_dict_to_file(self, dic_path: str, dict_type: str = "sakura") -> None:
        ...
        if dict_type == "sakura":
            ...
            with open(dic_path, "w", encoding="utf8") as f:
                f.write(self.dict_str.replace("->", "    ").replace(" #", "    "))
        elif dict_type == "json":
...
class SakuraTranslator(BaseTranslator):
    ...
    _CHAT_SYSTEM_TEMPLATE_009 = "你是一个轻小说翻译模型，可以流畅通顺地以日本轻小说的风格将日文翻译成简体中文，并联系上下文正确使用人称代词，不擅自添加原文中没有的代词。"
    _CHAT_SYSTEM_TEMPLATE_100 = "你是一个轻小说翻译模型，可以流畅通顺地以日本轻小说的风格将日文翻译成简体中文，并联系上下文正确使用人称代词，不擅自添加原文中没有的代词。"

    _CHAT_SYSTEM_TEMPLATE_GALTRANSL_V1 = "你是一个视觉小说翻译模型，可以通顺地使用给定的术语表以指定的风格将日文翻译成简体中文，并联系上下文正确使用人称代词，注意不要混淆使役态和被动态的主语和宾语，不要擅自添加原文中没有的代词，也不要擅自增加或减少换行。"
    ...
    def _setup_translator(self):
        ...
        sakura_version = (
            self.sakura_version if self.sakura_version != "galtransl-v1" else "1.0"
        )
        self.sakura_dict = SakuraDict(self.dict_path, self.logger, sakura_version)
    ...
    def detect_and_calculate_repeats(
        self, s: str, threshold: int = 20, remove_all=True
    ) -> Union[bool, str, int, str, int]:
        """
        ...
        for pattern_length in range(1, len(s) // 2 + 1):
            ...
            for match in re.finditer(pattern, s):
                ...
                if current_count >= threshold:
                    self.logger.warning(
                        f"检测到重复模式: {current_pattern}，重复次数: {current_count}"
                    )
    ...
    def _format_prompt_log(self, prompt: str) -> str:
        gpt_dict_raw_text = self.sakura_dict.get_dict_str_within_text(
            prompt, self.force_apply_dict
        )
        prompt_009 = "\n".join(
            [
                "System:",
    ...
    def _check_translation_quality(
        self, queries: List[str], response: str
    ) -> List[str]:
        """
        ...
        def _retry_translation(
            queries: List[str], check_func: Callable[[str], bool], error_message: str
        ) -> str:
            styles = [
                "precise",
                "normal",
        ...
        if self.detect_and_calculate_repeats(
            "".join(queries), self.repeat_detect_threshold
        )[0]:
            self.logger.warning(
        ...
        actual_threshold = max(
            max(self.detect_and_calculate_repeats(query)[4] for query in queries),
            self.repeat_detect_threshold,
        )

        if self.detect_and_calculate_repeats(response, actual_threshold)[0]:
            response = _retry_translation(
                queries,
                lambda r: self.detect_and_calculate_repeats(r, actual_threshold)[0],
        ...
        if not self.check_align(queries, response):
            response = _retry_translation(
                queries,
                lambda r: not self.check_align(queries, r),
        ...
        while True:
            if OPENAPI_V1_API:
                try:
                    ...
                    self.logger.warning("Sakura因文件不存在而进行重试。")
                    time.sleep(30)
                except TimeoutError:
                    timeout_attempt += 1
                ...
                try:
                    ...
                    server_error_attempt += 1
                    if server_error_attempt >= self.retry_attempts:
                        self.logger.warning(e, "Sakura翻译失败。返回原始文本。")
                        return "\n".join(prompt)
                    self.logger.warning(
    ...
    def _request_translation(self, input_text_list):
        ...
        gpt_dict_raw_text = self.sakura_dict.get_dict_str_within_text(
            raw_text, self.force_apply_dict
        )
        if self.sakura_version == "0.9" or gpt_dict_raw_text == "":
            messages = [
                {"role": "system", "content": f"{self._CHAT_SYSTEM_TEMPLATE_009}"},
                {"role": "user", "content": f"将下面的日文文本翻译成中文：{raw_text}"},
            ]
        elif self.sakura_version == "1.0":
            messages = [
                {"role": "system", "content": f"{self._CHAT_SYSTEM_TEMPLATE_100}"},
                {
                    "role": "user",
        ...
        if OPENAPI_V1_API:
            client = openai.Client(api_key="sk-114514", base_url=self.api_base)
