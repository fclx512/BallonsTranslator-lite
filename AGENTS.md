...
| `ui/batch_inpaint.py` | 批量「简单背景」纯色修复（工作台任务，D3／D34）：把 `modules/inpaint/base.py::classify_simple` 的判据跑遍全书页，**简单块纯色覆盖、复杂块完全不动**（`inpaint` 的 `only_simple` 模式，不加载模型），判据一律算原图、只写 `inpainted/` 层、跑完重载当前页、不标脏；整批走 `ui/batch_ops.py::BatchOperation` 的版本撤回 |
...
| `ui/glossary_agent_panel.py` | **泛用工作台的容器**（设计见 `docs/技术实现/AI辅助功能_设计与实现.md` 的「工作台」部分；D19～D27：工作台＝任务容器，不是聊天窗）：一级导航 `ui/glossary_agent_panel.py::WorkbenchTaskNav` 六个任务（误识别清理→合并→扩张→背景修复→术语提取→剧情摘要，前四项缀「还有 N 个未处理」）+ 每任务一页 + 底部日志条与「撤销上次批量」；`GlossaryAgentWorker` 仍是术语／剧情的权威草稿持有者，日志落面板底部（D19 已砍 Chat）；批量任务的事务外壳在此建好注入（带主窗口 `_sync_and_commit_project`／`_sync_block_data`，见设计 §14 的 D40 第 ① 步）。入口＝左栏 `ui/mainwindowbars.py::LeftBar` 的 workbenchChecker（D25 单入口），跳转走 `ui/mainwindow.py::on_workbench_jump`（复用具脏页惰性重渲的 pageList 链路，D26） |
...
| `scripts/` | `verify.py`, `check_docs.py`, `check_syntax.py`, `check_audit.py`, `check_showcase.py`, `qm_compile.py`, `i18n_check.py`, `workbench_recalc.py`（工作台参数复算台，六个只读子命令）；`scripts/probes/` 是真机探针（内存归因/释放阶梯、区域再检测验收），自带说明 |
