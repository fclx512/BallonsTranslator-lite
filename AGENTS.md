...
| `utils/block_actions.py` | 框级 AI 动作（OCR 校正/重译）注册表 + vision 调用 + 选中跟随工具栏；载荷在主线程按前端观感组装；详见 `docs/基础速查/AI辅助标签体系使用说明.md` |
...
| `ui/workbench_tasks.py` | 工作台四个批量任务的数据侧适配层（`MisreadTask`/`MergeTask`/`ExpandTask`/`SimpleInpaintTask`，**无 QWidget**）；**界面不写几何、不碰 `proj.pages`、不绕开 `ui/batch_ops.py`** |
...
| `scripts/` | `verify.py`, `check_docs.py`, `check_syntax.py`, `check_audit.py`, `check_showcase.py`, `qm_compile.py`, `i18n_check.py`, `trim_daily_log.py`（daily_log 3 天窗口清理，pre-commit 钩子调用）、`workbench_recalc.py`（工作台参数复算台，六个只读子命令）、`workbench_render.py`（工作台各任务页渲染成 PNG，目视验收布局用）；`scripts/probes/` 是真机探针，自带说明 |
